...
async def start_cmd(message: types.Message):
    await message.answer(
        ...
        "Savollar blok ichida va variantlari har safar tasodifiy (random) aralashtiriladi!"
    )
...
async def set_block_and_show_timer(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        f"✅ {block_idx+1}-Blok tanlandi ({len(block_questions)} ta savol blok ichida aralashtirildi).\n"
